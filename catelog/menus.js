...
function init(){
	...
	document.getElementById("close").onclick = closeOldMenu;
...
function changeMenu(){
	...
	activeMenu.style.width= "240px";
	activeMenu.style.clip = "rect(0px, 240px, 0px, 0px)";
...
function rollDown(){
	clipHgt = clipHgt + 10;
	if(clipHgt < 400){
		activeMenu.style.clip = "rect(0px,240px," + clipHgt + "px,0px)";
	}else{
		clearInterval(timeID);
		clipHgt = 0;
	}
